...
- [x] API plane: removed the legacy WebAuthn mock admin gate in `authMiddleware` — it crashed on body-less requests (`Cannot destructure 'verification' of req.body` → 500) for any unmatched `/api/admin/*`/`/admin/*` path and was an always-reject mock; real admin routes already run earlier via adminRouter's Bearer `adminAuth`. Unmatched admin paths now return the normal Express 404
