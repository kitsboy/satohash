...
export default function ZKRedactionTool({ isOpen, onClose, contract }) {
  ...
  useEffect(() => {
    const visible = segments
      .map((s) => (s.redacted ? BLOCK.repeat(s.text.length) : s.text))
      .join('')
  ...
  const toggleSeg = useCallback(
    (i) => setSegments((p) => p.map((s, idx) => (idx === i ? { ...s, redacted: !s.redacted } : s))),
    []
  )
    ...
    <AnimatePresence>
      ...
      >
        ...
        >
          ...
          {/* Live hash banner */}
          <motion.div
            animate={{ background: hashFlash ? 'rgba(99,102,241,0.12)' : 'var(--bg-primary)' }}
          ...
          >
            <Hash
              size={13}
              style={{ color: hashFlash ? '#818cf8' : 'var(--text-secondary)', flexShrink: 0 }}
            />
