...
function Sep() {
  return (
    <span className="shrink-0 px-2 text-[8px]" style={{ color: 'rgba(255,255,255,0.12)' }}>◆</span>
  )
}
...
function TickerItem({ icon: Icon, label, value, suffix, highlight }) {
  return (
    <span className="inline-flex shrink-0 items-center gap-2 px-5">
      <Icon size={10} style={{ color: highlight ? 'var(--accent-gold)' : 'var(--accent-active)', flexShrink: 0 }} />
      <span className="text-[9px] font-bold tracking-[0.12em] uppercase" style={{ color: 'var(--text-secondary)' }}>
        {label}
      </span>
      <span className="font-mono text-[10px] font-black" style={{ color: highlight ? 'var(--accent-gold)' : 'var(--text-primary)' }}>
        {value ?? '—'}{suffix}
      </span>
    </span>
  )
...
export default function MempoolTicker({ compact = false }) {
  ...
  if (compact) {
    return (
      <div className="flex items-center gap-3">
        <span className="flex items-center gap-1.5 rounded-full border px-2 py-0.5 text-[9px] font-bold tracking-widest uppercase"
          style={{ border: '1px solid var(--border)', background: 'rgba(255,255,255,0.03)', color: 'var(--text-secondary)' }}>
          <span className="inline-block h-1.5 w-1.5 rounded-full"
            style={{ background: pulse ? 'var(--accent-active)' : 'var(--accent-success)', boxShadow: `0 0 6px ${pulse ? 'var(--accent-active)' : 'var(--accent-success)'}`, transition: 'background 0.3s' }} />
          {height ? `#${height.toLocaleString()}` : '—'}
        </span>
        {fees.fastestFee && (
          <span className="flex items-center gap-1 text-[9px] font-bold" style={{ color: 'var(--text-secondary)' }}>
  ...
  const items = data ? (
    <>
      ...
      {txCount !== null && <><Sep /><TickerItem icon={Radio} label="Mempool" value={txCount.toLocaleString()} suffix=" txs" /></>}
      {vBMB !== null && <><Sep /><TickerItem icon={TrendingUp} label="Backlog" value={vBMB} suffix=" MB" /></>}
    </>
  ) : null

  return (
    <div className="relative flex w-full items-center overflow-hidden"
      style={{ height: '26px', borderBottom: '1px solid var(--border)', background: 'var(--bg-secondary)' }}>
      {/* Live label */}
      <div className="z-10 flex shrink-0 items-center gap-2 border-r px-4"
        style={{ borderColor: 'var(--border)', background: 'var(--bg-secondary)', height: '100%' }}>
        <span className="h-1.5 w-1.5 rounded-full"
          style={{ background: 'var(--accent-success)', boxShadow: '0 0 6px var(--accent-success)', animation: 'pulse 2s infinite' }} />
        <span className="text-[8px] font-black tracking-[0.25em] uppercase" style={{ color: 'var(--text-secondary)' }}>Live</span>
      </div>
      ...
      {data ? (
        ...
        <div className="flex flex-1 items-center px-4">
          <span className="text-[9px] font-bold tracking-widest animate-pulse uppercase" style={{ color: 'var(--text-secondary)' }}>
