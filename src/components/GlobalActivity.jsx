...
export default function GlobalActivity() {
    ...
    return (
        <div style={{ padding: '24px', background: 'white', borderRadius: '32px', border: '1px solid #e2e8f0', shadow: '0 20px 50px -12px rgba(0,0,0,0.05)' }}>
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '24px' }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
                    <div style={{ width: '10px', height: '10px', borderRadius: '50%', background: '#ef4444', animation: 'pulse 2s infinite' }} />
                    <h4 style={{ margin: 0, fontSize: '14px', fontWeight: '900', color: '#0f172a', textTransform: 'uppercase', letterSpacing: '0.1em' }}>Global Proof Pulse</h4>
                </div>
                    ...
                    {activities.map((activity) => (
                        ...
                        >
                            <div style={{
                                ...
                                height: '40px',
                                borderRadius: '12px',
                                background: activity.type === 'anchor' ? 'rgba(99, 102, 241, 0.1)' : 'rgba(16, 185, 129, 0.1)',
                                display: 'flex',
                                alignItems: 'center',
                                justifyContent: 'center',
                                color: activity.type === 'anchor' ? '#4f46e5' : '#10b981'
                            }}>
                                {activity.type === 'anchor' ? <Shield size={18} /> : activity.type === 'signature' ? <Zap size={18} /> : <Globe size={18} />}
                            </div>

                            <div style={{ flex: 1 }}>
                                <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '4px' }}>
                                    <span style={{ fontSize: '12px', fontWeight: '800', color: '#0f172a', textTransform: 'uppercase' }}>
                                        {activity.type === 'anchor' ? 'Anchored' : activity.type === 'signature' ? 'Signed' : 'Verified'}
                                    </span>
                                    <span style={{ fontSize: '10px', fontWeight: '800', color: '#94a3b8' }}>{activity.time}</span>
                                </div>
                                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                                    <code style={{ fontSize: '10px', color: '#64748b' }}>{activity.hash}</code>
                                    <span style={{ fontSize: '10px', fontWeight: '700', color: '#94a3b8' }}>{activity.location}</span>
                                </div>
                            </div>
                        </motion.div>
                    ))}
            ...
            <div style={{ marginTop: '24px', textAlign: 'center' }}>
                <button style={{ background: 'none', border: 'none', color: '#4f46e5', fontSize: '12px', fontWeight: '800', cursor: 'pointer', textTransform: 'uppercase', letterSpacing: '0.05em' }}>
                    View Live Topology →
                </button>
            </div>
