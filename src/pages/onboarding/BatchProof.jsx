        ...
        <div className="page" style={{ background: 'var(--color-surface)', paddingTop: '100px' }}>
            <div className="container">
                <div className="page-header text-center" style={{ marginBottom: '40px' }}>
                    ...
                    <h1 style={{ fontWeight: '950', fontSize: 'clamp(28px, 6vw, 48px)', letterSpacing: '-0.04em', marginBottom: '12px', color: 'var(--color-text-primary)' }}>
                        Bulk Proof Manager
                    </h1>
                    <p style={{ color: 'var(--color-text-secondary)', fontSize: '18px', maxWidth: '600px', margin: '0 auto', fontWeight: '600' }}>
                        Anchor entire document libraries to Bitcoin in a single cryptographic operation.
                    </p>
                </div>

                <div style={{ display: 'grid', gridTemplateColumns: files.length > 0 ? '1fr 350px' : '1fr', gap: '32px' }}>
                    {/* Main Area */}
                            ...
                            <div style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
                                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '8px' }}>
                                    ...
                                    {!showResults && (
                                        <button
                                            onClick={() => setFiles([])}
                                            style={{ background: 'none', border: 'none', color: '#ef4444', fontWeight: '700', fontSize: '14px', cursor: 'pointer' }}
                                        >
                                            Clear All
                                        </button>
                                    )}
                                ...
                                {files.map((file) => (
                                    <Card key={file.id} style={{ padding: '20px', display: 'flex', alignItems: 'center', gap: '20px', background: 'white' }}>
                                        <div style={{
                                            width: '48px',
                                            height: '48px',
                                            borderRadius: '12px',
                                            background: file.status === 'anchored' ? 'rgba(34, 197, 94, 0.1)' : '#f1f5f9',
                                            display: 'flex',
                                            alignItems: 'center',
                                            justifyContent: 'center',
                                        ...
                                        <div style={{ flex: 1 }}>
                                            <div style={{ fontWeight: '850', fontSize: '16px', color: '#000000' }}>{file.name}</div>
                                            <div style={{ fontSize: '13px', color: '#333333', fontFamily: 'var(--font-mono)', marginTop: '4px', fontWeight: '700' }}>
                                                Hash: {file.hash.substring(0, 16)}...
                                            </div>
                                        </div>
                                        <div style={{ textAlign: 'right', display: 'flex', alignItems: 'center', gap: '12px' }}>
                                            <span style={{ fontSize: '13px', color: '#94a3b8', fontWeight: '600' }}>{file.size}</span>
                                            {!showResults ? (
                                                <button
                                                ...
                                                <div style={{ color: '#22c55e', display: 'flex', alignItems: 'center', gap: '4px', fontSize: '14px', fontWeight: '700' }}>
                    ...
                    {files.length > 0 && (
                        <div style={{ display: 'flex', flexDirection: 'column', gap: '24px' }}>
                            <Card style={{ padding: '32px', background: '#0f172a', color: 'white', border: 'none' }}>
                                <h3 style={{ color: 'white', fontSize: '20px', fontWeight: '850', marginBottom: '24px' }}>Batch Stats</h3>
                                <div style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
                                    ...
                                    <div style={{ display: 'flex', flexDirection: 'column', gap: '8px', padding: '16px 0', borderTop: '1px solid rgba(255,255,255,0.1)' }}>
                                        <div style={{ color: '#94a3b8', fontSize: '12px', fontWeight: '700', textTransform: 'uppercase', letterSpacing: '1px' }}>Merkle Root</div>
                                        <div style={{ fontFamily: 'var(--font-mono)', fontSize: '11px', color: '#6366f1', wordBreak: 'break-all' }}>
                                            3c9a...bd2e
                                        </div>
                                    </div>
                                </div>
                                ...
                                {!showResults ? (
                                    ...
                                    <Button
                                        ...
                                        fullWidth
                                        size="large"
                                        onClick={() => { setFiles([]); setShowResults(false); }}
                                        style={{ marginTop: '32px', height: '60px', background: 'rgba(255,255,255,0.1)', border: 'none', color: 'white' }}
                                    >
                                        Start New Batch
                                    </Button>
                                )}
                            ...
                            <div style={{
                                ...
                                <div style={{ display: 'flex', alignItems: 'center', gap: '12px', color: '#4f46e5', marginBottom: '12px' }}>
                                    <Activity size={20} />
                                    <span style={{ fontWeight: '800', fontSize: '14px' }}>Network Efficiency</span>
                                </div>
                                <p style={{ fontSize: '13px', color: '#64748b', lineHeight: '1.5', margin: 0 }}>
                                    Batch anchoring saves up to 98% in network fees by committing thousands of documents in a single Merkle Tree transaction.
                                </p>
                            </div>
                        </div>
                    )}
