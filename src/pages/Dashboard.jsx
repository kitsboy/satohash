            ...
            <div className="grid lg:grid-cols-3 gap-8">
                    ...
                    >
                        {!file ? (
                            ...
                            <div className="flex-1 flex flex-col items-center justify-center text-center p-12 bg-gradient-to-br from-slate-50 to-white rounded-xl border border-slate-100/50">
                                ...
                                <p className="text-slate-500 max-w-sm">Drag any document here to begin the cryptographic anchoring process.</p>
                            </div>
                        ) : (
                            // Success State
                            <div className="flex-1 flex flex-col bg-white rounded-xl p-8 relative">
                                ...
                                <div className="flex items-start gap-4 mb-8">
                                    ...
                                    <div>
                                        ...
                                        <p className="text-sm font-mono text-slate-400 mt-1">SHA-256: 8f434346648f6b96df89dda901c5176b10a6...</p>
                                    </div>
                                </div>

                                <div className="grid sm:grid-cols-3 gap-4 mt-auto">
                                    <button onClick={downloadOTS} className="flex flex-col items-center justify-center p-4 border border-slate-200 rounded-xl hover:bg-slate-50 transition-colors group">
                                    ...
                                    <button onClick={() => generatePDF(file)} className="flex flex-col items-center justify-center p-4 border border-slate-200 rounded-xl hover:bg-slate-50 transition-colors group">
                                    ...
                                    <button onClick={handleEmail} className="flex flex-col items-center justify-center p-4 border border-slate-200 rounded-xl hover:bg-slate-50 transition-colors group">
                ...
                <div className="space-y-8">
                            ...
                            {recentFiles.map((item, i) => (
                                <div key={i} className="flex items-center justify-between p-3 hover:bg-slate-50 rounded-lg transition-colors cursor-pointer group">
                                    <div className="flex items-center gap-3">
                                        ...
                                        <div>
                                            <p className="text-sm font-medium text-slate-700 group-hover:text-indigo-600">{item.name}</p>
                                            <p className="text-xs text-slate-400">{item.date}</p>
                                        </div>
                                    </div>
                                    <span className={`text-[10px] font-bold px-2 py-0.5 rounded-full ${item.status === 'Confirmed' ? 'bg-green-100 text-green-700' : 'bg-amber-100 text-amber-700'}`}>
                                        {item.status}
                                    </span>
                                </div>
                            ))}
                    ...
                    <div className="glass-card p-6 bg-gradient-to-br from-indigo-600 to-purple-700 text-white">
                        ...
                        <p className="text-sm text-indigo-100 mb-4">Satohash is 100% free and open source. Help us keep the lights on.</p>
