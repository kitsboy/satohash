...
import { Download, Mail, Copy, Check, Shield, EyeOff, Eye, ChevronDown, ChevronUp, ExternalLink, Binary, QrCode } from 'lucide-react';
    ...
    return (
        <div className="page">
            <div className="container container-narrow">
                ...
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 180px', gap: 'var(--spacing-lg)', marginBottom: 'var(--spacing-xl)' }}>
                    ...
                    <Card style={{ padding: 'var(--spacing-md)', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', background: 'white' }}>
                        <QRCodeSVG
                            ...
                            includeMargin={false}
                        />
                        <span style={{ fontSize: '10px', fontWeight: '850', color: 'var(--color-primary)', marginTop: '12px', textAlign: 'center', textTransform: 'uppercase', display: 'flex', alignItems: 'center', gap: '4px' }}>
                            <QrCode size={12} />
                            Scan to Verify
                        </span>
                    </Card>
                ...
                <Card style={{ marginBottom: 'var(--spacing-lg)', border: '1px solid var(--color-primary-light)', background: 'var(--color-primary-xlight)' }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: 'var(--spacing-md)' }}>
                        <div style={{ padding: 'var(--spacing-sm)', background: 'white', borderRadius: '50%', display: 'flex' }}>
                            <Shield size={24} className="text-primary" />
                        </div>
                        <div style={{ flex: 1 }}>
                            <h4 className="mb-1" style={{ fontSize: 'var(--text-md)', fontWeight: 'bold' }}>Network Confirmation Status</h4>
                ...
                {contract.merkleTree && (
                    <div style={{ marginBottom: 'var(--spacing-xl)' }}>
                        <div className="flex items-center gap-2 mb-4 text-slate-800">
                            ...
                            <h4 style={{ margin: 0, fontSize: 'var(--text-md)', fontWeight: 'bold' }}>Protocol Visualization</h4>
                        </div>
                ...
                <Card style={{ marginBottom: 'var(--spacing-xl)' }}>
                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 'var(--spacing-md)' }}>
                        <h4 style={{ margin: 0, fontSize: 'var(--text-md)', fontWeight: 'bold' }}>Selective Redaction (Merkle Proof)</h4>
                        <button
                    ...
                    {isRedactionOpen && (
                        <div style={{ marginTop: 'var(--spacing-md)' }}>
                            <p className="text-secondary" style={{ fontSize: 'var(--text-sm)', marginBottom: 'var(--spacing-lg)' }}>
                                Toggle parts of the document to reveal. You can prove the existence of this document even if some parts are hidden.
                            </p>

                            <div style={{ display: 'flex', flexDirection: 'column', gap: 'var(--spacing-sm)', maxHeight: '300px', overflowY: 'auto', marginBottom: 'var(--spacing-lg)', paddingRight: 'var(--spacing-sm)' }}>
                                {contract.merkleTree?.atoms.map((atom, idx) => (
                                    <div
                                        ...
                                        style={{
                                            ...
                                            border: visibleAtoms.has(idx) ? '1px solid var(--color-border)' : '1px solid transparent',
                                    ...
                                    >
                                        {visibleAtoms.has(idx) ? <Eye size={16} className="text-primary" /> : <EyeOff size={16} className="text-secondary" />}
                                        <div style={{
                                            flex: 1,
                                            fontSize: 'var(--text-sm)',
                            ...
                            <Button
                                ...
                                style={{ width: '100%' }}
                                onClick={() => {
                                    const revealedAtoms = Array.from(visibleAtoms).sort((a, b) => a - b).map(idx => ({
                                        index: idx,
                                    ...
                                    const blob = new Blob([JSON.stringify(redactedPackage, null, 2)], { type: 'application/json' });
