...
const SECTIONS = [
  {
    ...
    features: ['POST /api/v1/timestamp — Create a proof', 'GET /api/v1/verify/:id — Verify a proof', 'GET /api/v1/price — Current pricing', 'Rate-limited to 100 req/min for free tier']
  },
  {
    icon: Server, title: 'Webhooks',
    desc: 'Receive real-time notifications when proofs are confirmed on-chain.',
    features: ['Configurable webhook URLs', 'HMAC-SHA256 signature verification', 'Automatic retry with backoff', 'Supports JSON and form-encoded payloads']
  },
  {
    icon: Globe, title: 'WordPress Integration',
    desc: 'Embed Satohash proofs in any WordPress site with a simple shortcode.',
    features: ['[satohash_proof id="..."] shortcode', 'Customizable iframe dimensions', 'Works with any WordPress theme', 'No plugin required — paste the code']
  },
  {
    icon: Smartphone, title: 'Mobile-Ready',
    desc: 'Proof pages are fully responsive and work on any device.',
    features: ['Responsive verification pages', 'PWA installable on iOS/Android', 'QR code sharing for proofs', 'Deep link support']
  }
]
  ...
  return (
    <div className="rounded-xl border border-[var(--border)] bg-[var(--bg-primary)] overflow-hidden">
      <div className="flex items-center justify-between border-b border-[var(--border)] bg-[var(--surface-raised)] px-4 py-2">
        <span className="text-[10px] font-bold tracking-widest text-[var(--accent-gold)] uppercase">{label}</span>
        <button
          onClick={() => { navigator.clipboard.writeText(code); setCopied(true); setTimeout(() => setCopied(false), 2000) }}
          className="flex items-center gap-1.5 text-[10px] font-bold text-[var(--text-tertiary)] hover:text-[var(--accent-gold)] transition-colors"
        >
          {copied ? <Check size={12} /> : <Copy size={12} />}
          {copied ? 'Copied!' : 'Copy'}
        </button>
      </div>
      <pre className="overflow-x-auto p-4 text-[11px] leading-relaxed text-[var(--text-secondary)] font-mono whitespace-pre">{code}</pre>
    </div>
  )
}
    ...
    <div className="min-h-screen bg-[var(--bg-primary)]">
      <header className="border-b border-[var(--border)] bg-[var(--bg-navbar)]/95 px-6 py-4 backdrop-blur-md">
        <div className="mx-auto flex max-w-6xl items-center gap-4">
          ...
          <span className="text-[10px] font-bold tracking-[0.25em] text-[var(--accent-gold)] uppercase">Integrations</span>
        </div>
      </header>
      ...
      <section className="border-b border-[var(--border)] px-6 pt-20 pb-16">
        <div className="mx-auto max-w-3xl text-center">
          ...
          <p className="mx-auto max-w-xl text-sm leading-relaxed text-[var(--text-secondary)]">
            Add Bitcoin-anchored proof of existence to your apps, websites, and workflows.
            REST API, webhooks, WordPress, and CLI — pick your path.
          </p>
        </div>
      </section>
      ...
      <section className="mx-auto max-w-6xl px-6 py-12">
        <div className="grid gap-6 sm:grid-cols-2">
          {SECTIONS.map((sec, i) => {
            const Icon = sec.icon
            return (
              <div key={i} className="rounded-2xl border border-[var(--border)] bg-[var(--surface-raised)] p-6 transition-all hover:border-[var(--accent-gold)]">
                ...
                <p className="mb-4 text-xs leading-relaxed text-[var(--text-secondary)]">{sec.desc}</p>
                <ul className="space-y-1.5">
                  {sec.features.map((f, j) => (
                    <li key={j} className="flex items-start gap-2 text-[11px] text-[var(--text-secondary)]">
      ...
      <section className="px-6 py-12">
        <div className="mx-auto max-w-xl text-center">
          ...
          <p className="mb-6 text-sm text-[var(--text-secondary)]">
            Generate API keys from the developer dashboard. Each key has scoped permissions and can be revoked independently.
          </p>
          <Link to="/developer"
