...
class NotaryMesh {
    constructor() {
        ...
        logger.info(`🔗 Ethereum bridge initialized with mock provider (wallet: ${this.ethWallet.address})`);
    }
    ...
    async propagate(stampId, hash, ipfsCid = '') {
        logger.info(`🌐 Propagating Proof ${stampId} to ${this.peers.length} peers and Ethereum bridge...`);
        ...
        try {
            if (ipfsCid) {
                ...
                logger.info(`⛓️ Proof ${stampId} bridged to Ethereum: TX ${txHash} at block ${receipt.blockNumber}`);

                // Emit event simulation
                this.ethContract.emit('ProofBridged', ethers.getBytes(hash), ipfsCid, this.ethWallet.address);
            } else {
                logger.warn(`⚠️ Skipping Ethereum bridge: no IPFS CID for ${stampId}`);
            }
        } catch (bridgeErr) {
            logger.warn(`⚠️ Ethereum bridge failed for ${stampId}: ${bridgeErr.message}. Continuing with mesh propagation.`);
    ...
    async extendWithMockTx(stampId, hash) {
        ...
        logger.info(`✅ Mock extension TX for ${stampId}: ${mockTx.hash} at block ${mockTx.blockNumber}`);
        return mockTx;
    }
