...
adminRouter.get('/stats', adminAuth, (req, res) => {
  ...
  const confirmedStamps = db
    .prepare("SELECT count(*) as count FROM timestamps WHERE status = 'confirmed'")
    .get().count
  const pendingStamps = db
    .prepare("SELECT count(*) as count FROM timestamps WHERE status = 'pending'")
    .get().count
...
adminRouter.get('/throttle-metrics', adminAuth, async (req, res) => {
  try {
    ...
    res.json({
      ...
      avgHitsPerHour:
        Object.values(timeSeries).reduce((a, b) => a + b, 0) / Object.keys(timeSeries).length || 0,
