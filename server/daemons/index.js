  ...
  const complianceInterval = setInterval(
    async () => {
      try {
        ...
        const recentStamps = db
          .prepare('SELECT * FROM timestamps WHERE created_at > datetime("now", "-5 minutes")')
          .all()
        if (recentStamps.length > 0) {
          const highRisk = recentStamps.filter(() => Math.random() < 0.1) // 10% chance
      ...
      try {
        ...
        if (blockedIps.length > 0) {
          const recentBlocks = await Promise.all(
            blockedIps.slice(-5).map(async (key) => {
              const reason = await redis.get(key)
              return {
                ip: key.replace('blocklist:', ''),
                reason,
                timestamp: new Date().toISOString()
              }
            })
