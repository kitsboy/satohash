...
function waitForOutput(child, needle, timeoutMs = 30000) {
  return new Promise((resolve, reject) => {
    let acc = ''
    const matcher = needle instanceof RegExp ? needle : new RegExp(needle.replace(/[.*+?^${}()|[\]\\]/g, '\\$&'))
    const onData = (chunk) => {
      acc += chunk.toString()
      if (matcher.test(acc)) {
        cleanup()
    ...
    const onExit = (code) => {
      cleanup()
      reject(new Error(`child exited (code=${code}) before '${needle}' was seen. Output so far:\n${acc.slice(-4000)}`))
    }
    const onTimeout = () => {
      cleanup()
...
describe('backup.js', () => {
  ...
  it('produces exactly one restorable backup and resolves to an existing file', async () => {
  ...
  it('boots the real server and stays alive after the boot backup completes (regression: unawaited backup + immediate close)', async () => {
    ...
    try {
      ...
      await waitForOutput(child, 'v5 jobs: pending-stamp prune scheduled')
